...
class CodeEditWorkflow:
    ...
    async def run(self, request: str) -> CodeEditResult:
        ...
        diff_targets = lenient_diff_target_paths(diff_text)
        explicit_targets = mentioned_workspace_files(self.workspace_root, request)
        if not diff_targets and not explicit_targets:
            return CodeEditResult(
                request=request,
                pack=pack,
                diff_text=diff_text,
        ...
        targets = diff_targets or explicit_targets or target_paths
