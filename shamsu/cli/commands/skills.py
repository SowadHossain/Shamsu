"""`/skills` - what skill files this workspace exposes, and which would fire.

Lived in `shamsu/skills/cli.py`. Same reason as `/mcp`: rendering is a command
...
def handle(context: CommandContext, argument: str) -> None:
    ...
    command = parts[0].strip().lower() if parts else "list"
    ...
    if command == "show":
        if len(parts) < 2 or not parts[1].strip():
            ...
            return
        _print_skill_detail(catalog, parts[1].strip(), console)
        return
    if command in {"explain", "suggest"}:
        if len(parts) < 2 or not parts[1].strip():
            console.print("[red]Usage: /skills explain <prompt>[/red]")
            return
        _print_skill_explanation(workspace, parts[1].strip(), catalog, console)
        return
    console.print(
        "[red]Usage: /skills list | show <name> | explain <prompt> | suggest <prompt>[/red]"
    )
...
COMMANDS = (
    Command(
        name="/skills",
        summary="List skills, show one, or explain which would fire for a prompt",
        handler=handle,
        completions=("list", "show ", "explain "),
    ),
)
