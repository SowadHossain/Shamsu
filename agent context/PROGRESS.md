...
- Current next focus: the full A-G reliability/autonomy roadmap is done. Remaining open follow-ups: a real-tokenizer swap for `context/budget.py` if/when a vendored tokenizer asset becomes available, wiring council mode into `CodeEditWorkflow` (currently only `BugFixWorkflow`), and trying `/autonomy on` in real use before considering it as a possible future default. Milestone 6 Dev C status/log/progress, release docs, and final safety audit from the original MVP plan are still outstanding separately.
