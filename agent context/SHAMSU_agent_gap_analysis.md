...
| I3 | Evals are single-sample (noisy) + default tier only | Measurement | MEDIUM | ◑ sampling + light tier baselined (heavy needs model pulls) |
...
>
> **Light tier baselined for the first time (2026-07-17).** First honest run: 9/12, with three cases at a consistent 0/3. Diagnosing each (by printing the failing finals, not by reasoning) split them three ways:
> - `run_command_verify` 0/3 → **3/3**: a **check-wording bug**. The 3B answered *"The command succeeded."* — correct — and the check grepped for `"success"`, which "succeeded" does not contain. The check now accepts "succeed" AND requires the artifact `py_compile` actually leaves (`__pycache__/ok.*.pyc`), so it can no longer be passed by self-report alone. Default tier re-measured 3/3 under the stricter check — the 7B really was running the command.
> - `ask_user_clarifies` 0/3 → **~1/3 (wobbles)**: the failing final was a bare ``` ``` `` — a stray unpaired fence that survived stripping and dodged the empty-response correction. `_strip_tool_artifacts` now collapses fence-only answers to empty (3 boundary tests), after which the case produces textbook questions on some rolls ("Which file do you want to add the comment to? (client or web)") and skips asking on others. The residual wobble is the 3B's ask-threshold, not an agent bug.
> - `rename_file_via_move_tool` stays **0/3 — a real tier limit, precisely located in the PLANNER**. Full trace: the light planner decomposes "rename old→new" into *"create new_name.py with same content; ensure no other files are affected"* — a **copy, not a rename** — and the 3B executor follows the plan to the letter (read → create → "Rename successful."), leaving both files on disk. The 7B executor survives a mediocre plan; the 3B is plan-literal. An executor-side "rename" special-case would game the eval, and planner wording is measured-fragile ground — so this stays a named limit. (The read-failure correction now lists real workspace files instead of sending small models on a `find_file` hunt — landed with a test since the old coaching was observed being *echoed as the final answer*, but it does not rescue this case.)
>
> **Final regenerated baselines (post-fixes, 3 samples): default 11/12, light 9/12** (`BENCHMARK.md` / `BENCHMARK-light.md`; the reading-rules footer is now emitted by `render_report` itself, since regeneration clobbers hand-appended caveats). The clarify/ask family is the light tier's soft spot **as a family** — individual ask-cases flip between 0/3 and 2/3 across runs of identical code, so read them as "3B asks sometimes", not case-by-case. Default's one FAIL (`rename` 1/3) is FLAKY-flagged in both of today's runs and sits at the majority line; the light tier's core file work (qa / create / edit / bugfix / run / both plan-grounding cases) is solid.
